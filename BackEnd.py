...
def get_candidate_documents_and_scores_bm25_for_len1(query_to_search, words, index, idf_dict, docs_len, blob, avgdl, bucket_name, b=0.75, k1=1.5):
...
def get_candidate_documents_and_scores_bm25(query_to_search, words, index, idf_dict, docs_len, blob, avgdl, bucket_name,
    ...
    # Create a lock for accessing shared data
    candidates_lock = threading.Lock()

    # Create threads for processing each word in the query
    threads = []
    for word in np.unique(query_to_search):
        thread = threading.Thread(target=process_term, args=(word, words, index, idf_dict, docs_len, blob, avgdl,
                                                             bucket_name, candidates, candidates_lock,
                                                             b, k1))
        thread.start()
